    ...
    std::map<const std::string, bool> &minus_n_commands_mangle)
{
    ...
    std::unique_ptr<MangleTableCompiler_nft> mangle_compiler(
        new MangleTableCompiler_nft(objdb , fw,
                                    ipv6_policy , oscnf,
    ...
    if ( (mangle_rules_count = mangle_compiler->prolog()) > 0 )
    {
        ...
        // rules. We'll call
        // MangleTableCompiler_nft::flushAndSetDefaultPolicy
    ...
    std::unique_ptr<PolicyCompiler_nft> policy_compiler = createPolicyCompiler(
